...
The hotkey acts on a selection — what the user pointed at. With nothing selected it converts the word being typed before the caret: the `AutoRun` buffer (word + trail), which the key monitor feeds on both platforms whether or not auto-correct is on, and which every caret-moving event resets (click, Cmd/Ctrl shortcut, navigation, focus change). An empty buffer — after a space, a click, an arrow — means nothing to convert. Other unselected text is never guessed at: no line grab, no word-at-the-caret window. `caretWord(_:)` stays in the engine for the auto mode.
