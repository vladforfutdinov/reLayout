...
The hotkey acts on a selection — what the user pointed at. With nothing selected it converts `AutoRun.hotkeyTarget`: the word being typed before the caret, or the last word followed only by spaces (erased with those spaces and retyped with them). The key monitor feeds the run on both platforms whether or not auto-correct is on. The target ends at any other key after the spaces (which starts the next word), at Tab/Return, at any conversion (auto fix, hotkey, undo), and at every caret-moving event (click, Cmd/Ctrl shortcut, navigation, focus change); backspacing all the spaces makes the word the one being typed again. No target means nothing to convert. Other unselected text is never guessed at: no line grab, no word-at-the-caret window. `caretWord(_:)` stays in the engine for the auto mode.
...
- `WinInput.swift` — `SendInput` writes: text as real key presses of the target layout after switching to it (`typeText`; VK_PACKET can't be batched — a busy app reads one character several times, undocumented), paced Unicode events as the fallback, Backspace, Shift+Left; the Ctrl+C fallback read with clipboard save/restore, `pumpWait`, focus/console helpers.
- `WinUIA.swift` + `native/relayout_uia.cpp` — UI Automation: the selection, the field snapshot for the Enter follow-up, the password-field flag. C++ because Swift has no COM.
- `WinTheme.swift` + `native/relayout_theme.cpp` — light/dark: DWM title bar, the system's dark control styles, theme colors for background/text/separators, dark tray menu (uxtheme ordinal 135).
- `WinUpdate.swift` + `native/relayout_http.cpp` — the update check (GitHub latest release over WinHTTP, which Swift's WinSDK module lacks).
...
Only built when host OS is Windows (`#if os(Windows)` gate in `Package.swift`); macOS `swift test` ignores the executable target. Selection is read through UI Automation (`windows/native/relayout_uia.cpp`, in the C++ target `RelayoutNative` — Swift has no COM); the clipboard is the fallback for controls without UIA text, and is saved and restored around it. `WinAuto.swift` is the opt-in auto-correct mode: the keyboard hook feeds typed characters into a word buffer, a boundary (space/Tab) runs the shared `decideAutoTarget`, and the fix is backspaced in and retyped. Trigram models ship as `trigram/<lang>.txt` next to the exe. Short words follow the macOS neighbour rule, and the Enter follow-up uses a second UIA call (field length, caret, selection, the text before the caret) with the engine's `enterOutcome`. A deny-list of executables (Settings > Exceptions — a list with icons, "Exclude current app" from a foreground WinEvent hook, "Choose…") and the UIA password-field flag gate it per app and per field. UI strings come from the macOS `Localizable.strings`, shipped as `lang/<code>.lproj` next to the exe and read with the engine's `parseStrings` (`WinLoc.swift`); Settings mirrors the macOS window (logo, login + language, a click-to-record hotkey field where a double tap is recorded as two taps, auto-correct with an ⓘ tooltip and Exceptions…, centered footer); a language change or `WM_DPICHANGED` rebuilds it. The hook runs on the main thread, so every wait in the retype path uses `pumpWait` (pumps messages), never `Sleep`. Releases: on a `v*` tag `build-windows` attaches the installer and the portable SFX (x64, arm64, unsigned) to the GitHub Release. `WinUpdate.swift` checks GitHub's latest release a minute after start and daily (WinHTTP via `relayout_http.cpp`), announces a newer one once in the tray and adds a menu item to its page; nothing is downloaded. Dev builds skip it.
